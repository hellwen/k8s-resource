...
if [ ${#PEERS[@]} -eq 1 ]; then
    ...
    echo 1 > "${MY_ID_FILE}"
    cp "${CFG_BAK}" "${CFG}"
    echo "server.1=${PEERS[0]}:2191:2192;2181" > "${CFG}"
...
for peer in "${PEERS[@]}"; do
    ...
    if [[ "${peer}" == *"${HOSTNAME}"* ]]; then
      ...
      MY_NAME=${peer}
      echo $i > "${MY_ID_FILE}"
      echo "server.${i}=${peer}:2191:2192:observer;2181" >> "${CFG_BAK}"
    else
      echo "server.${i}=${peer}:2191:2192:participant;2181" >> "${CFG_BAK}"
    fi
done
...
$ZK_HOME/bin/zkCli.sh reconfig -add "server.$MY_ID=$MY_NAME:2191:2192:participant;2181"
